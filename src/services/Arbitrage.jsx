...
export default function Arbitrage() {
  ...
  const [minNet, setMinNet] = useState(0.3);
    ...
    <section className="page">
      ...
      <div className="card">
        {loading ? "Loading…" : filtered.length===0 ? "No opportunities" :
          filtered.map((o, i) => (
            <div key={o.id || i} style={{display:"grid",gridTemplateColumns:"100px 1fr 1fr 90px 120px",gap:12,padding:"8px 0",borderBottom:"1px solid var(--line)"}}>
              <div>{o.symbol}</div>
              ...
              <div style={{color:"#94a3b8"}}>{o.ts ? new Date(o.ts).toLocaleString() : ""}</div>
            </div>
          ))
        }
      </div>
