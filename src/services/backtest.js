...
export default function Backtest() {
  ...
  const [source, setSource] = useState("binance"); // NEW: mock | binance
  ...
  const [upgrade, setUpgrade] = useState(null); // NEW: апселл

  async function run() {
    setLoading(true); setErr(null); setUpgrade(null);
    try {
      ...
      // обработка 402 UPGRADE_REQUIRED
  ...
  return (
    <section className="page">
      <h1 className="page-title">
        {t("pages.backtest?.title") || "Backtest (RSI/MACD, EMA) Ч MVP"}
      </h1>

      <div className="card" style={{
        display: "grid",
        gridTemplateColumns: "repeat(7, minmax(160px, 1fr))", gap: 12, alignItems: "end"
      }}>
        <label>Source
      ...
      {upgrade && (
        <div className="card" style={{ borderColor: "var(--warn)" }}>
          ...
          <a className="btn" href="/pricing">Go to Pricing</a>
        </div>
      )}
