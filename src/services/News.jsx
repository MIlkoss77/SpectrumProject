  ...
  return (
    <section className="page">
      ...
      <div className="card" style={{display:"flex", gap:8, marginBottom:12}}>
        {langs.map(l => <button key={l} className={"pill"+(lang===l?" selected":"")} onClick={()=>setLang(l)}>{l.toUpperCase()}</button>)}
        {["all","low","med","high"].map(k => <button key={k} className={"pill"+(impact===k?" selected":"")} onClick={()=>setImpact(k)}>{k}</button>)}
      </div>
